...
The app takes a JSON file as input — each entry represents one task segment for one participant. Place your data in the following structure before running:

- `analysis.json` — the main dataset file you upload in the app
- `videos/` — folder containing participant video files (.mp4, .mov, etc.)
- `gesture_grids/` — folder containing gesture grid images, organized by participant subfolder
  - `gesture_grids/participant1/grid_001.png`
  - `gesture_grids/participant2/grid_001.png`
...
**Step 3 —** Upload your JSON file from the sidebar. Make sure your `videos/` and `gesture_grids/` folders are in the same directory where you run the command.
